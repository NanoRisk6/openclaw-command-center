...
async function loadChart() {
  if (!Chart) {
    const chartModule = await import('https://cdn.jsdelivr.net/npm/chart.js@4.4.3/dist/chart.umd.js');
...
async function fetchSOLPrice() {
  try {
    const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd', {cache: 'default'});
    const data = await res.json();
    return data.solana ? data.solana.usd : 86;
  } catch (e) {
    // SOL price fetch failed, using mock
    return 86;
  }
}
...
async function updateData() {
  try {
    ...
    const pipelineRes = await fetch('/data/pipeline_status.json', {cache: 'force-cache'});
    ...
    const treasuryRes = await fetch('/data/treasury-state.json', {cache: 'force-cache'});
    const treasury = await treasuryRes.json();
    const treasury_current = treasury.treasury_current || 0;
    const treasury_target = treasury.treasury_target || 1;
    const cpuUsage = Math.min((treasury_current / treasury_target * 100), 100) || 0;

    // Cache hit for Memory
    const cacheHitStr = treasury.cache_hit || '65';
    const memUsage = parseFloat(cacheHitStr.replace('%', '')) || 65;

    // Market state (placeholder)
    const marketRes = await fetch('/data/market_state.json', {cache: 'force-cache'});
    const market = await marketRes.json();

    // Derivatives for Disk/Net
    const derivRes = await fetch('/data/derivatives_derived.json', {cache: 'force-cache'});
...
async function createCharts() {
  ...
  memChart = new ChartClass(memCtx, {
    type: 'line',
    data: {
      labels: [],
      datasets: [{
  ...
  diskChart = new ChartClass(diskCtx, {
    type: 'line',
    data: {
      labels: [],
      datasets: [{
  ...
  netChart = new ChartClass(netCtx, {
    type: 'line',
    data: {
      labels: [],
      datasets: [
...
function updateChartsMock() {
  ...
  // Memory
  memChart.data.labels.push(now);
  memChart.data.datasets[0].data.push(Math.floor(Math.random() * 80) + 10);
  if (memChart.data.labels.length > 30) {
    memChart.data.labels.shift();
    memChart.data.datasets[0].data.shift();
  }
  ...
  // Disk
  diskChart.data.labels.push(now);
  diskChart.data.datasets[0].data.push(Math.floor(Math.random() * 80) + 10);
  if (diskChart.data.labels.length > 30) {
    diskChart.data.labels.shift();
    diskChart.data.datasets[0].data.shift();
  }
  ...
  // Network
  netChart.data.labels.push(now);
  netChart.data.datasets[0].data.push(Math.random() * 100);
  netChart.data.datasets[1].data.push((Math.random() - 0.5) * 200);
  if (netChart.data.labels.length > 30) {
    netChart.data.labels.shift();
    netChart.data.datasets[0].data.shift();
    netChart.data.datasets[1].data.shift();
